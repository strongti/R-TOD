...
int write_result(void)
{
    ...
    fprintf(fp, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n", "e_fetch", "b_fetch", "d_fetch",
            ...
            "slack", "e2e_delay", "fps", "c_sys", "IFG", "n_obj", "fetch_offset");
    for(int i=0;i<OBJ_DET_CYCLE_IDX;i++)
    {
        ...
        fprintf(fp, "%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%d,%d, %d\n", e_fetch_array[i], b_fetch_array[i],d_fetch_array[i], 
                e_infer_cpu_array[i], e_infer_gpu_array[i], d_infer_array[i], e_disp_array[i], b_disp_array[i], d_disp_array[i], 
                slack[i], e2e_delay[i], fps_array[i], cycle_time_array[i], inter_frame_gap_array[i], num_object_array[i], fetch_offset);
    }
    fclose(fp);
...
int get_fetch_offset(void)
{
    ...
    if (cnt < (ZERO_SLACK_CYCLE_OFFSET - 1))
    {
        printf("\nMIN cycle time : %f\n", MIN(s_min, (1000./fps)));
    ...
    else if (cnt == (ZERO_SLACK_CYCLE_OFFSET - 1)) 
    {
	...
	offset = (int)(s_min - e_fetch_max - b_fetch_max);
        ...
        printf("Calculated your fetch offset: %d ms\n", offset);

        fp=fopen("zero_slack.txt","w+");
        fprintf(fp, "\nMIN cycle time : %f\n", s_min);
        fprintf(fp, "MAX e_fetch : %f\n", e_fetch_max);
        fprintf(fp, "MAX b_fetch : %f\n", b_fetch_max);
	fprintf(fp, "Fetch Offset : %d\n", offset);
	//fetch_offset = offset;
...
void push_data(void)
{
    ...
    printf("fetch delay : %f", d_fetch);
    printf("inference delay : %f", d_infer);
    printf("e2e_delay : %f", e2e_delay[cnt-CYCLE_OFFSET]);
    //printf("end_disp : %f \n", end_disp);    
